...
import { LibUpdateAddressCandidateModalComponent } from './lib-update-address-candidate-modal/lib-update-address-candidate-modal.component';
import { GlobalApisService } from '../../../services/lib-global-apis.service';
...
@Component({
  selector: 'lib-candidate-personal-details',
  templateUrl: './lib-candidate-personal-details.component.html',
  styleUrls: ['./lib-candidate-personal-details.component.scss']
})
export class LibCandidatePersonalDetailsComponent implements OnInit {
  ...
  constructor(
    ...
    private _exGlobal: GlobalApisService,
  ...
  getPersonalDetails(type:string = 'N'){
    this._candidateServe.getCandidatePersonalDetails(this.apiBaseUrlCand).subscribe(
      res => {
  ...
  callMasterAPI() {
    forkJoin([
      this._exGlobal.getNationalityMaster(this.apiBaseUrlMaster),
      this._exGlobal.getMaritalStatusMaster(this.apiBaseUrlMaster),
      this._exGlobal.getSkillsList(this.apiBaseUrlMaster),
  ...
  addUpdateAddress(type: string = ''): void {
    if(!this.isFinalSumbit){
      ...
      const dialogRef = this.dialog.open(LibUpdateAddressCandidateModalComponent, {
        width: '500px',
