...
import { LIBCANDIDATE_COMMON } from 'projects/ats-lib/src/lib/core/constant/lib-candidate-common.const';
...
@Component({
  selector: 'lib-candidate-education-details-form-modal',
  templateUrl: './lib-candidate-education-details-form-modal.component.html',
  styleUrls: ['./lib-candidate-education-details-form-modal.component.scss']
})
export class LibCandidateEducationDetailsFormModalComponent implements OnInit {
  ...
  public eduGradeMaster: any = LIBCANDIDATE_COMMON.eduGradeMaster;
  public isSponsored: any = LIBCANDIDATE_COMMON.sponsoredMaster;
  ...
  constructor(
    public dialogRef: MatDialogRef<LibCandidateEducationDetailsFormModalComponent>,
    @Inject(MAT_DIALOG_DATA) public data: any,
    private _fb: UntypedFormBuilder,
    private _share: LibShareService,
    private _exGlobal: LibExternalUserGlobalApiService,
    private _candidateServ: LibCandidateService,
  ...
  getDegreeNames() {
    this._exGlobal.getDegreeNamesMaster(this.apiBaseUrlMaster).subscribe(
      res => {
  ...
  getCollegeList() {
    this._exGlobal.getCollegeNamesMaster(this.apiBaseUrlMaster).subscribe(
      res => {
  ...
  getUniversityList() {
    this._exGlobal.getUniversityNamesMaster(this.apiBaseUrlMaster).subscribe(
      res => {
  ...
  getSpecializationList() {
    this._exGlobal.getSpecializationListMaster(this.apiBaseUrlMaster).subscribe(
      res => {
  ...
  getEducationTypeList() {
    this._exGlobal.getEducationTypeListMaster(this.apiBaseUrlMaster).subscribe(
      res => {
  ...
  getLevelNamesList() {
    this._exGlobal.getLevelListMaster(this.apiBaseUrlMaster).subscribe(
      res => {
  ...
  getDisciplineList() {
    this._exGlobal.getDisciplineListMaster(this.apiBaseUrlMaster).subscribe(
      res => {
  ...
  submitForm(form: any) {
    ...
    if (this.updateEduDetailsForm.valid) {
      ...
      let body = {
        ...
        formDate: formValue?.dateFrom ? LibGlobalMethod.formatDate(formValue?.dateFrom) : '',
        toDate: formValue?.dateTo ? LibGlobalMethod.formatDate(formValue?.dateTo) : '',
      ...
      if (this.data?.type == 1) {
        this._candidateServ.postEducationDetails(this.apiBaseUrlCand,body).subscribe(
          res => {
        ...
        this._candidateServ.updateEducationalDetails(this.apiBaseUrlCand,this.data?.id, body).subscribe(
          res => {
            this._share.showAlertSuccessMessage.next(res);
            this.dialogRef.close(true);
