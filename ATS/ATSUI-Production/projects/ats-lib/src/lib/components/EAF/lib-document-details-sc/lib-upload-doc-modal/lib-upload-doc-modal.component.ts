...
import { LIBFILE_UPLOAD } from 'projects/ats-lib/src/lib/core/constant/lib-common.const';
...
import { LibGlobalCommonMethodService } from 'projects/ats-lib/src/lib/services/lib-global-common-method.service';
import { LibShareService } from 'projects/ats-lib/src/lib/services/lib-share.service';
import { forkJoin } from 'rxjs';
@Component({
  selector: 'lib-upload-doc-modal',
  templateUrl: './lib-upload-doc-modal.component.html',
  styleUrls: ['./lib-upload-doc-modal.component.scss']
})
export class LibUploadDocModalComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<LibUploadDocModalComponent>,
    @Inject(MAT_DIALOG_DATA) public data: any,
    private _fb: UntypedFormBuilder,
    private _messageService: LibShareService,
    private _commonMethodServe: LibGlobalCommonMethodService,
 ...
 getEmploymentDetails(page:boolean = false) {
  ...
  getSubChildDocType() {
    this._exGlobal.getDocumentsChildSubcategoryMaster(this.apiBaseUrlMaster).subscribe(
      res => {
  ...
  fileChangeEvent(event: any, index: number): void {
    if (event.target.value) {
      ...
      reader.onload = () => {
        ...
        if (blob.size > LIBFILE_UPLOAD.FILE_SIZE) {
  ...
  submitdocUploadForm(form: UntypedFormGroup) {
    if(this.data?.element?.id !== 1 && this.data?.element?.id !== 2){
      ...
      if (form.valid) {
        ...
        // }
        this._candidateserve.uploadDocumentsBulk(this.apiBaseUrlCand,formData).subscribe(
          res => {
      ...
      if (form.valid) {
        ...
        for(let i=0;i<this.formFieldData.length; i++){
          ...
          apiCalls.push(this._candidateserve.uploadDocumentsBulk(this.apiBaseUrlCand,formData))
        }
