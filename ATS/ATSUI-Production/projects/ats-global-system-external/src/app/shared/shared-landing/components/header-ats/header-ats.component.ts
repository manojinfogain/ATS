...
import { AtsCommonFuncService } from 'projects/ats-global-system-external/src/app/core/common/ats-common-func.service';
...
import { GlobalApisService } from 'projects/ats-global-system-external/src/app/core/services/global-apis.service';
...
export class HeaderAtsComponent implements OnInit {
  ...
  getLocation() {
    ...
    if (token) {
      this._globalApiServe.getLocationList().subscribe(
        res => {
          let filterById = [1, 2, 3, 4, 5, 6, 11, 10, 16];
  ...
  public hideShowForOtpVerification: boolean = false;
  hideHeaderSideBar() {
    debugger
    ...
    this._router.events.subscribe(e => {
      if (e instanceof NavigationEnd) {
        ...
        if (url == "otp-verification") {
          this.hideShowForOtpVerification = true;
        }else{
          this.hideShowForOtpVerification = false;
        }
      }
    });
