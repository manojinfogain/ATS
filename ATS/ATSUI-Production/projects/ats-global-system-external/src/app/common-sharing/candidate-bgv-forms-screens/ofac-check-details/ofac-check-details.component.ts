...
@Component({
  selector: 'app-ofac-check-details',
  templateUrl: './ofac-check-details.component.html',
  styleUrls: ['./ofac-check-details.component.scss']
})
export class OfacCheckDetailsComponent implements OnInit {
  @Input() appearance: string = 'outline';
  @Input() formClass: string = 'form-outline-ats';
  public isRequired: boolean = true;
  @Input() public OFACCheckDetailsForm: UntypedFormGroup = new UntypedFormGroup({});
  ...
  ngOnInit(): void {
    this.callMasterAPI();
    this.InitOFACCheckDetailsForm();
    setTimeout(() => {
          Object.keys(this.OFACCheckDetailsForm.controls).forEach(key => {
            const control = this.OFACCheckDetailsForm.get(key);
   ...
   * get PAersonal
   */
  public addedOFACCheckDet: any = [];
  public idTypeName: string = '';
  getPersonalDetails(type: string = 'N') {
    this._candidateServe.GetBGVOFACDetails().subscribe(
      res => {
        this.addedOFACCheckDet = res['data'][0];  
        this.idTypeName = this.idTypeList.find((elm: any) => elm.Id == this.addedOFACCheckDet?.DocumentTypeId)?.DocumentName;
        if (this.addedOFACCheckDet?.Id) {
          this.setAddedValueToForm(this.addedOFACCheckDet);
        }
        else {
  ...
  setValueToForm(data: any) {
    ...
    this.OFACCheckDetailsForm.patchValue({
  ...
  InitOFACCheckDetailsForm() {
    this.OFACCheckDetailsForm = this._fb.group({
      Id: [null],
  ...
  getControl(name: string) {
    return this.OFACCheckDetailsForm.get(name);
  }
