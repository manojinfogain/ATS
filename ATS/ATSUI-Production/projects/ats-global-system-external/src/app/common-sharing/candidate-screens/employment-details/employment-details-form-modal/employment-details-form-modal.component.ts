...
import { requiredMsgPrefix, salaryMinMaxLoc } from 'projects/ats-global-system-external/src/app/core/constant/common.const';
...
export class EmploymentDetailsFormModalComponent implements OnInit {
  ...
  getDocumentList() {
    forkJoin([
      this._exGlobal.getDesignationList(),
      this._exGlobal.getFunctionList(),
      this._exGlobal.getIndustryList(),
      this._exGlobal.GetSkillNames(),
      this._exGlobal.GetCountryNames()
    ]).subscribe(
      res => {
  ...
  empFormInit() {
    ...
    this.addAndRemoveValidatorsFieldsForLeader();
  }

  /**non-mandatory ctc for leadership */
  addAndRemoveValidatorsFieldsForLeader() {
    if (this.data?.isLeadershipOnboard) {
      this.getControl('joiningCtc').clearValidators();
      this.getControl('leavingCtc').clearValidators();
      this.getControl('reasonForLeave').clearValidators();

    } else {
      this.getControl('joiningCtc').setValidators([Validators.required]);
      this.getControl('leavingCtc').setValidators([Validators.required]);
      this.getControl('reasonForLeave').setValidators([Validators.required]);
    }
    this.getControl('joiningCtc').updateValueAndValidity();
    this.getControl('leavingCtc').updateValueAndValidity();
    this.getControl('reasonForLeave').updateValueAndValidity();
  }
  ...
  submitEmpDetailsForm(form: any) {
    ...
    if (this.employmentDetailsForm.valid) {
      ...
      if (formValue['durationFrom']) {
        formValue['fromDate'] = GlobalMethod.formatDate(formValue['durationFrom']);
      }
      if (formValue['durationTo']) {
        formValue['toDate'] = GlobalMethod.formatDate(formValue['durationTo']);
      }
      ...
      if (this.data?.type == 1) {
        this._candidateServe.addEmploymentDetails(formValue).subscribe(
          res => {
        ...
        this._candidateServe.updateEmploymentDetails(this.data?.id, formValue).subscribe(
          res => {
            this._share.showAlertSuccessMessage.next(res);
            this.dialogRef.close(true);
          }
  ...
  public endDateLabel: string = 'To Date';

  selectTillDate(e: any) {
    this.durationToControl.reset();
    if (e.checked == true) {
      this.durationToControl.disable()
      this.durationToControl.clearValidators();
      if (this.data?.isLeadershipOnboard) {
        //this.durationToControl.setValue(null);
        this.endDateLabel = 'Last Working Date';
        //    this.durationToControl.enable()
        // this.durationToControl.addValidators([Validators.required]);
        // this.durationToControl.patchValue(this.todayDate);
        //  this.durationToControl.clearValidators();
      } else {
        this.endDateLabel = 'To Date';
