...
@Component({
  selector: 'app-leadership-onboard-details',
  templateUrl: './leadership-onboard-details.component.html',
  styleUrls: ['./leadership-onboard-details.component.scss']
})
export class LeadershipOnboardDetailsComponent implements OnInit {
  ...
  hideShowActionButton() {
    ...
    if (this.selectedTabIndex != 6) {
      ...
      if (getNextTabInfo['disabled']) {
        // if (this.selectedTabIndex == 4 || this.selectedTabIndex == 0) {
         if (this.selectedTabIndex == 0) {
          this.isNextBtnShow = true;
        }
        else {
  ...
  */
  ...
  //  let salaryDetailsForm: FormGroup = this.salaryDetailsCom?.updateSalDetailsForm;
...
debugger
        ...
        if (otherDetailsform.valid) {
          // this.confirmAlertSubmit(otherDetailsform);
            if (!this.isBGVFormSubmitted && this.userData?.ISBGVFormsEnable === 'Y') {
                      ...
                      //this.confirmAlertSubmit(otherDetailsform);
                        if (
                          !this.isBGVFormSubmitted &&
                          this.userData?.ISBGVFormsEnable === 'Y'
                        ) {
                          let elm = { title: 'Complete BGV Form First' };
                          elm['src'] = 'EAF';
                          elm['msg'] =
                            'Kindly complete the Background Verification form First to submit the Employment Application Form.';
                          const dialogRef = this.dialog.open(
                            ConfirmIfBgSubmittedComponent,
                            {
                              panelClass: [
                                'ats-model-wrap',
                                'onboard-consent-modal',
                              ],
                              data: elm,
                              disableClose: true,
                            }
  ...
  switchToNext() {
    debugger
    let getTabsLen = (this.getAllTab.length) - 1;
