...
@Component({
  selector: 'app-otp-verification-video-match',
  templateUrl: './otp-verification-video-match.component.html',
  styleUrls: ['./otp-verification-video-match.component.scss']
})
export class OtpVerificationVideoMatchComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<OtpVerificationVideoMatchComponent>,
  ...
  ngOnInit(): void {
    ...
    const control = <UntypedFormArray>this.videoSubmitFormGroup.controls['item'];
    for (let i = 0; i < 6; i++) {
      control.push(this.initItemRow(null));
    }
    if (this.data?.queryParam && this.data?.userEmpId) {
     // this.hiddenEmail = this._globalMethodServe.hideEmail(this.data?.email);
     this.hiddenEmail = this._globalMethodServe.hideEmail(this.data?.userEmail);
   ...
   */
   ...
   sendOTPtoCandidate() {
     this._candServe.SendOTPtoVideoMatcher(this.data?.queryParam,this.data?.userEmpId).subscribe(
       res => {
 ...
 */
  ...
  submitForm(form: UntypedFormGroup) {
    ...
    if(form.valid){
      ...
      let param = `param=${this.data?.queryParam}&EmpId=${this.data?.userEmpId}&otp=${enterdOtp}`;
      this._candServe.submitOtpVideoMatching(param).subscribe(
        res => {
