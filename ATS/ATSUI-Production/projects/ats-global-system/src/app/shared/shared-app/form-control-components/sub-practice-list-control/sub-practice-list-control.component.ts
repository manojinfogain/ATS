...
@Component({
  selector: 'app-sub-practice-list-control',
  templateUrl: './sub-practice-list-control.component.html',
  styleUrls: ['./sub-practice-list-control.component.scss']
})
export class SubPracticeListControlComponent implements OnInit, OnChanges {
  @Input() public IdControl: UntypedFormControl;
  public FilterCtrlSubPractice: UntypedFormControl = new UntypedFormControl();
  public searchInputSubPractice: string;
  public subPracticeList: any = [];
  ...
  ngOnInit(): void {
    if (this.PracticeIds?.length != 0) {
      ...
      this.getSubPracticeListByID(ids);
    }
    if (!this.isMultiple) {
  ...
  //get list by id
  getSubPracticeListByID(idList:string): void {
    console.log(typeof(idList));
    this._globalServe.getSubPracticeListByID(idList).subscribe(
      res => {
        let filterByStatus = res['data'];
        this.subPracticeList = filterByStatus;
        this.FilterCtrlSubPractice.valueChanges.subscribe(
          val => {
            this.searchInputSubPractice = val;
  ...
  optionClick() {
    ...
    this.select.options.forEach((item: MatOption) => {
      ...
      if (
        ...
        !this.allSelcount &&
        !this.FilterCtrlSubPractice?.value?.length
      ) {
        newStatus = true;
      }
    });
