...
@Component({
  selector: 'app-recruiter-list-control',
  templateUrl: './recruiter-list-control.component.html',
  styleUrls: ['./recruiter-list-control.component.scss']
})
export class RecruiterListControlComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.type == "hiringManager"){
      this.HiringManagerList();
    }
    
  }

  recruiterList() {
    this._globalServe.getRecruiter()
    .subscribe(
      res => {
  ...
  HiringManagerList() {
    this._globalServe.getHiringManagerList()
    .subscribe(
      res => {
        this.recList =  res['data'];
        this.FilterCtrl.valueChanges.subscribe(
