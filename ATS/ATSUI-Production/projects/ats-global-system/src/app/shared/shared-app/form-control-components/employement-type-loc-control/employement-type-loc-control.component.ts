...
@Component({
  selector: 'app-employement-type-loc-control',
  templateUrl: './employement-type-loc-control.component.html',
  styleUrls: ['./employement-type-loc-control.component.scss']
})
export class EmployementTypeLocControlComponent implements OnInit {

  @Input() ContractTypeTitle: string = 'Contract Type';
  @Input() public contractTypeControlOptUS: UntypedFormControl = new UntypedFormControl();
  ...
  ngOnInit(): void {
    ...
    this.contractTypeControlOptUS.valueChanges.subscribe(
      v=>{
        if(v === null){
  ...
  getContractList() {
    const locId = this.companyLocation ? this.companyLocation : null;
    this._globalServe.GetContractTypesForUS(3).subscribe(
      res => {
        if (this.contractIdsUS.length == 0) {
          this.contactList = res['data'];
        } else {
          let filteredContType = res.data.filter(t => {
            return this.contractIdsUS.indexOf(t.ID) !== -1;
          });
