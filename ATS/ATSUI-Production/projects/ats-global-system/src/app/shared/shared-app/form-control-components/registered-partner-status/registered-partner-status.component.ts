...
@Component({
  selector: 'app-registered-partner-status',
  templateUrl: './registered-partner-status.component.html',
  styleUrls: ['./registered-partner-status.component.scss']
})
export class RegisteredPartnerStatusComponent implements OnInit {
  @Input() regisPartnerStatusTitle: string = 'Partner Status';
  @Input() public registeredPartnerStatusControlOpt: UntypedFormControl = new UntypedFormControl();
  ...
  ngOnInit(): void {
    ...
    this.registeredPartnerStatusControlOpt?.valueChanges.subscribe(
      v => {
        if (v === null) {
