...
export class MessageComponent implements OnInit,OnDestroy {
  ...
  public msgErrorParamSubscription:Subscription;
  constructor(
  ...
  ngOnInit(): void {
     ...
     * error alert
     */
      this.msgErrorParamSubscription = this._share.showAlertErrorParamMessage.subscribe(
        get=>{
          this.alertModalErrorWithParam(get);
        }
      );
    /***
     * sesssion
  ...
  */
  ...
  alertModalErrorWithParam(data){
    this._snackBar.openFromComponent(MessageSnackBarErrorComponent, {
      duration: (data?.durationInSeconds?data?.durationInSeconds:this.durationInSeconds) * 1000,
      horizontalPosition: this.horizontalPosition,
      verticalPosition: this.verticalPosition,
      panelClass:'ats-snackbar',
      data:data?.message
