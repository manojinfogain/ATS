...
@Component({
  selector: 'app-view-generate-offer',
  templateUrl: './view-generate-offer.component.html',
  styleUrls: ['./view-generate-offer.component.scss']
})
export class ViewGenerateOfferComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<ViewGenerateOfferComponent>,
  ...
  getCandidateDetails() {
    forkJoin([
      ...
      res => {
        ...
        if (this.offerAprDt?.JoiningLocationID == 4 || this.offerAprDt?.JoiningLocationID == 2 || this.offerAprDt?.JoiningLocationID == 5 || this.offerAprDt?.JoiningLocationID == 16) {
  ...
  getLocation() {
    this._globalApiServe.getLocationList().subscribe(
      res => {
        ...
        else if(this.data.DivisionID == 9){
           ids = [1, 2, 4, 5, 23];
        }
         else if (this.data.DivisionID == 1) {
          ids = [1, 2, 4, 5, 16];
        }
 ...
 */
  ...
  submitBodyFormData(body: any) {
    ...
    this._offerService.generateOffer(formData).subscribe(
      res => {
        this._share.showAlertSuccessMessage.next(res)
        this.dialogRef.close(true);
      }
