...
@Component({
  selector: 'app-upload-manual-offer-modal',
  templateUrl: './upload-manual-offer-modal.component.html',
  styleUrls: ['./upload-manual-offer-modal.component.scss']
})
export class UploadManualOfferModalComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<UploadManualOfferModalComponent>,
...
*/
  ...
  getCandidateDetails() {
    forkJoin([
      ...
      res => {
        ...
        this.getControl('offerNumber').patchValue(this.offerAprDt.offerNumber);
        this.getControl('DateOfJoining').patchValue(new Date(this.offerAprDt?.DateOfJoining));
        this.getControl('SudexoCoupen').patchValue(this.offerAprDt?.SudexoCoupen?this.offerAprDt?.SudexoCoupen.toString():0);
        this.setValidationForFinalCTc(this.offerAprDt);
        if (this.offerAprDt?.JoiningLocationID == 4 || this.offerAprDt?.JoiningLocationID == 2 || this.offerAprDt?.JoiningLocationID == 5) {
  ...
  getLoc(event:any){
    const locId:number = event.value;
    if (locId == 4 || locId == 2 || locId == 5) {
      this.sudCoupHide = true;
    }
    else {
      this.sudCoupHide = false;
  ...
  setValidationForFinalCTc(data: any) {
  ...
  getLocation() {
    this._globalApiServe.getLocationList().subscribe(
      res => {
        // let ids = [1, 2, 4, 5];
        let ids = [];
        if(this.data.DivisionID == 7 || this.data.DivisionID ==  1){
          ids = [1, 2, 4, 5, 16];
        }else{
          ids = [1, 2, 4, 5];
        }
 ...
 */
  ...
  submitBodyFormData(body: any) {
    ...
    /**
     * if bgv file selected
     */
    if (this.allFilesBGV.length != 0) {
      let bgvData = new FormData();
      bgvData.append('cid', body.cid);
      for (let i = 0; i < this.allFilesBGV.length; i++) {
        bgvData.append('file', this.allFiles[i]);
      }
      this._offerService.UploadBGVDocuments(bgvData).subscribe(
        res => {
         // this._share.showAlertSuccessMessage.next(res)
  ...
  uploadOfferletter(formData:FormData){
    formData.append('offer', this.offerLtterFile);
    this._offerService.uploadOffer(formData).subscribe(
      res => {
        this._share.showAlertSuccessMessage.next(res)
        this.dialogRef.close(true);
      }
  ...
  @ViewChild('offerletter') offerletter: ElementRef;
  fileUp(event: any) {
