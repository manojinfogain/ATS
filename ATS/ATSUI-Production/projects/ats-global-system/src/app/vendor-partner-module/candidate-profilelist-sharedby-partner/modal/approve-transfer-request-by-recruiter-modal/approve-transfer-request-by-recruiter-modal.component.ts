...
@Component({
  selector: 'app-approve-transfer-request-by-recruiter-modal',
  templateUrl: './approve-transfer-request-by-recruiter-modal.component.html',
  styleUrls: ['./approve-transfer-request-by-recruiter-modal.component.scss']
})
export class ApproveTransferRequestByRecruiterModalComponent implements OnInit {

  public approveTransferCandidateForm: UntypedFormGroup = new UntypedFormGroup({});

  constructor(
    public dialogRef: MatDialogRef<ApproveTransferRequestByRecruiterModalComponent>,
  ...
  approveTransferCandiHandler(form: UntypedFormGroup) {
    ...
    if (form.valid) {
      if(this.data['type'] === 1){
        this._PartnerServe.ApproveUnAttendentTransferProfilePartner(this.data.id, form.value.transferStatus, form.value.remark ? form.value.remark : null).subscribe(
          res => {
        ...
        this._PartnerServe.approveTransferedCandidatePartner(this.data.cid, form.value.transferStatus, form.value.remark ? form.value.remark : null).subscribe(
          res => {
            this._share.showAlertSuccessMessage.next(res);
            this.dialogRef.close(true);
          }
