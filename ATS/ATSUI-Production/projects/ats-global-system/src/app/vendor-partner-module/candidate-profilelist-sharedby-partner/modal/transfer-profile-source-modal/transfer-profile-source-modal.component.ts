...
@Component({
  selector: 'app-transfer-profile-source-modal',
  templateUrl: './transfer-profile-source-modal.component.html',
  styleUrls: ['./transfer-profile-source-modal.component.scss']
})
export class TransferProfileSourceModalComponent implements OnInit {
  public userData: any = {};
  ...
  constructor(
    public dialogRef: MatDialogRef<TransferProfileSourceModalComponent>,
  ...
  formInit() {
    this.formTransfer = this._fb.group({
      ProfileSorceId: [null, [Validators.required]],
      talendIdControl: [null],
...
*/
  ...
  getDataTalent(data) {
    ...
    let prvTalentId = this.data.thid;
  ...
  TransferProfileSource(form: any) {
    if (this.formTransfer.valid) {
      ...
      if (this.data['type'] === 1) {
       let dataObj = {
         'id':this.data.id,
         'isThidPresnet':formValue['talendIdControl'] ? 'Y' : 'N',
         'profileSource':formValue['ProfileSorceId'],
         'toThId':formValue['talendIdControl'],
         'remarks':formValue['remarksTransfer']
       }
        this._partnerService.transferUnattendedProfileSource(dataObj).subscribe(
          res => {
        ...
        formdata.append('CubeID', (formValue?.talendIdControl)?formValue.TCID: null);
        formdata.append('CubeClusterID',(formValue?.talendIdControl)?formValue?.cubeClusterId: '');
        formdata.append('CubeRoleID',  (formValue?.talendIdControl)?formValue.roleId: null);
        formdata.append('gradeId', (formValue?.talendIdControl)?formValue.gradeId: null);
        formdata.append('gradeBand', (formValue?.talendIdControl)?formValue.gradeBand: null);
        formdata.append('EmpUnitId', formValue?.EmpUnitId?formValue?.EmpUnitId:'');
        formdata.append('ProfileSorceId', formValue?.ProfileSorceId);
        formdata.append('IsThidPresent', (formValue?.talendIdControl)?'Y':'N');
        this._partnerService.transferProfileSource(formdata).subscribe(
          res => {
            this._share.showAlertSuccessMessage.next(res);
            this.dialogRef.close(true);
          }
