...
@Component({
  selector: 'app-transfer-candidate',
  templateUrl: './transfer-candidate.component.html',
  styleUrls: ['./transfer-candidate.component.scss']
})
export class TransferCandidateComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<TransferCandidateComponent>,
  ...
  TransferTalent(form: any) {
    if (this.formTransfer.valid) {
      ...
      if (this.data.type === 1) {
        ...
        this._partnerServe.transferCandiate(formValue).subscribe(
          res => {
