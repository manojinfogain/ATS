...
@Component({
  selector: 'app-us-hiring-tracker',
  templateUrl: './us-hiring-tracker.component.html',
  styleUrls: ['./us-hiring-tracker.component.scss'],
  providers: [DatePipe]
})
export class UsHiringTrackerComponent implements OnInit {

  displayedColumns = ['TalentId','Name', 'Phone', 'Email', 'EmploymentType','WorkVisaStatus','LegalEntity', 
  'Division','OfferType','TotalExperience','AccountName','PrimarRec','StartDate',
  'Status','Action'
  ];
  public userData: any = {};
  ...
  */
  ...
  getCandidateOffer(page: number, pageSize: number, search: string, sortParam: any) {
    ...
    this.bodyParam = body;
    let _apiUrl: string = 'USOffer/GetHiringTrackerReport';
...
downloadReportAsExel() {
    ...
    let _apiUrl: string = 'USOffer/ExportToExcelGetHiringTrackerReport';
...
* download report excel
*/
exportCandidateWiseHiringTrackeReport(cid:number) {
  // let bodyData = {
  //   ...this.bodyParam,
  //   page: 1,
  //   pageSize: this.paginationData?.Total,
  // }
  debugger
  let _apiUrl: string = `USOffer/ExportToExcelGetCandidateWiseHiringTrackerReport?cid=${cid}`;
  this.http.get(`${environment.apiMainUrlNet}${_apiUrl}`, { responseType: 'blob' }).subscribe(
    res => {
      saveAs(res, 'Candidate_Wise_Hiring_Tracker_Report.xls');
    },
    (error) => {
      this._share.showAlertErrorMessage.next('Something went wrong');
    }
