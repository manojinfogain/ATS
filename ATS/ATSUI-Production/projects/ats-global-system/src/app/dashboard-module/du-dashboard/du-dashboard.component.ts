...
@Component({
  selector: 'app-du-dashboard',
  templateUrl: './du-dashboard.component.html',
  styleUrls: ['./du-dashboard.component.scss']
})
export class DuDashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    /**
     * Get Delivery Wise details Api
     */
    this.getDeliveryHeadWiseDetails({ startDate: this.getPastdate() });
 ...
 */
  ...
  */  
  ...
  getDeliveryHeadWiseDetails(sortParam: any) {
    this.bodyParam = {};
    let body = {};
    ...
    this.bodyParam = body;
    this._dashServe.getDeliveryHeadWiseDetails(body).subscribe(
      res => {
        this.reportList = res['data'];
        if (this.reportList.length !== 0) {
          this.duChartDataFilter = this.reportList[0];
          this.DuControl.patchValue(this.reportList[0]?.DUID)
          this.legendTitle = this.reportList[0]?.DeliveryUnit || '';
          this.accountName = this.reportList[0]?.DeliveryUnit || '';
        }
  ...
  selectDuId(event: any) {
    ...
    if (event.value == 'all') {
      ...
      this.duChartDataFilter = this.reportList.filter(d => d.DUID == id)[0];
      this.legendTitle = this.duChartDataFilter?.DeliveryUnit || '';
      this.accountName = this.duChartDataFilter?.DeliveryUnit || '';
    }
  ...
  exportAsXLSX(): void {
    TableUtil.exportTableToExcel("exportTable", 'GDL Wise Report', `GDL Wise Report`);
  }
