...
import { VisibiltyTableViewCountComponent } from 'projects/ats-global-system/src/app/common-sharing/dashboard/visibilty-table-view-count/visibilty-table-view-count.component';
...
export class PmDashboardComponent implements OnInit {
  ...
  exportButtonMethod(){
    this._share.detectExcelExportPMWise.next({docName:"PM Wise Report"});
  }
  ...
  */  
   ...
   */
   ...
   getAccountOwnerWiseDetails(sortParam: any) {
    ...
    this.bodyParam = body;
    this._dashServe.getPMWiseDetails(body).subscribe(
      res => {
        this.reportList = res['data'];
        this.duChartDataFilter = this.reportList[0];
        this.DuControl.patchValue(this.reportList[0].ProjectID)
        this.accountName = this.reportList[0]?.ProjectName || '';
        console.log("ar",this.reportList[0]?.ProjectName || '' )
 ...
 /***
  ...
  selectDuId(event: any) {
    ...
    if (event.value == 'all') {
      ...
      this.duChartDataFilter = this.reportList.filter(d => d.ProjectID == id)[0];
      this.accountName = this.duChartDataFilter?.ProjectName || '';
