...
@Component({
  selector: 'app-account-owner-dasboard',
  templateUrl: './account-owner-dasboard.component.html',
  styleUrls: ['./account-owner-dasboard.component.scss']
})
export class AccountOwnerDasboardComponent implements OnInit {
  ...
  */  
  ...
  public bodyParam: any = {};
   getAccountOwnerWiseDetails(sortParam: any) {
    ...
    this.bodyParam = body;
    // this._dashServe.getHiringManagerWiseTHIDsDetails().subscribe(
    //   r=>{})
    this._dashServe.getAccountOwnerWiseDetails(body).subscribe(
      res => {
        this.reportList = res['data'];
        this.duChartDataFilter = this.reportList[0];
        this.DuControl.patchValue(this.reportList[0]?.AccountID)
        this.accountName = this.reportList[0]?.AccountName || '';
  ...
  selectDuId(event: any) {
    ...
    if (event.value == 'all') {
      ...
      this.duChartDataFilter = this.reportList.filter(d => d.AccountID == id)[0];
      this.accountName = this.duChartDataFilter?.AccountName || '';
  ...
  exportAsXLSX(): void {
    TableUtil.exportTableToExcel("exportAccTable", 'Account Wise Report', 'Account Wise Report');
  }
